...
class FurrierPropagation(AbstractPropagator):
    ...
    def _recalc_propagation_buffer(self):
        ...
        wavelength = self.wavelength.tensor.reshape(-1,1,1).to(self.device).to(self.accuracy.tensor_float)
        reflection = (self.reflection.tensor + 1j*self.absorption.tensor).reshape(-1,1,1).to(self.device).to(self.accuracy.tensor_complex)
